...
<?php
	...
	{
			 $sql = "select distinct ID from final_ID_Mapping where GeneName = '$protein[$i]'";
...
//<== If having only E3s without any substrate, will find all substrates recognized by these E3s: ---> Start
	//parameters for E3 ligases from the list of all possible UbiSubstrates
	$E3_ID_List2=array();
	$E3_AC_List2=array();
	$E3_GeneName_List2=array();
	$E3_group_List2=array();
	$E3_Ensembl_List2=array();
	$E3_PubMedID_List2=array();
	...
	if($d==count($E3_ID_List)){
	   for($i=0 ; $i < count($protein) ; $i++){
		...
		for($i=0 ; $i < count($sub_ID_List) ; $i++){
			$sql4 = "select AC from final_UbiSubstrate_Human where ID = '".$sub_ID_List[$i]."'";
			$result4 = mysql_query($sql4) or die("Query failed : " . mysql_error());
			$num_rows4 = mysql_num_rows($result4);
			if($num_rows4 > 0)
			  if($data4 = mysql_fetch_array($result4))
				 $sub_AC_List[count($sub_AC_List)] = $data4[0];
			
			// Get list of E3 ligases from List of all possible Ubisubstrates
			$sql5 = "select distinct E3_ID,E3_AC, GeneName, E3_group, Ensembl,PubMedID from final_E3_Human where E3_ID = '".$sub_ID_List[$i]."'";
			...
			if($num_rows5 > 0){
				if($data5 = mysql_fetch_array($result5)){
					$E3_ID_List2[count($E3_ID_List2)] = $data5[0];
					$E3_AC_List2[count($E3_AC_List2)] = $data5[1];
					$E3_GeneName_List2[count($E3_GeneName_List2)] = $data5[2];
					$E3_group_List2[count($E3_group_List2)]=$data5[3];
					$E3_Ensembl_List2[count($E3_Ensembl_List2)]=$data5[4];
					$E3_PubMedID_List2[count($E3_PubMedID_List2)]=$data5[5];
				}
			}
		}
		//print_r ($sub_AC_List);
	...
	// Check UbiSite-prediction for all inputted proteins
	$UbiProtein=array(); //=1 if UbiPredicted; =0 if NonUbiPredicted
	for($k=0; $k < $num_protein; $k++){
		$UbiProtein[$k]=0;
	}
	for($k=0; $k < $num_protein; $k++){
		$sql7 = "select distinct ID from final_UbiSite_Prediction where ID = '".$protein[$k]."'";       
		$result7 = mysql_query($sql7) or die("Query failed : " . mysql_error());
...
<script>
  ...
  var E3_ID_List2 = ["<?php echo join("\", \"", $E3_ID_List2); ?>"];
  var E3_AC_List2 = ["<?php echo join("\", \"", $E3_AC_List2); ?>"];
  var E3_GeneName_List2 = ["<?php echo join("\", \"", $E3_GeneName_List2); ?>"];
  var E3_group_List2 = ["<?php echo join("\", \"", $E3_group_List2); ?>"];
  var E3_Ensembl_List2 = ["<?php echo join("\", \"", $E3_Ensembl_List2); ?>"];
  var E3_PubMedID_List2 = ["<?php echo join("\", \"", $E3_PubMedID_List2); ?>"];
  ...
  var Sub_AC_via_ppi = ["<?php echo join("\", \"", $ppi_AC); ?>"];
  var Sub_GeneName_via_ppi = ["<?php echo join("\", \"", $ppi_GeneName); ?>"];

  var UbiProtein = ["<?php echo join("\", \"", $UbiProtein); ?>"]; // Is Ubi-Predicted or not?
  ...
  ready: function(){
		...
		if(input_proteins[0]!="")
		{
			for(var k=0 ; k < Sub_ID_via_ppi.length ; k++) // Add protein nodes
			{
				...
				node.data('text',Sub_GeneName_via_ppi[k]);
				node.data('IsProtein','true');
				node.data('IsE3','false');
				node.data('IsSub','false');
				node.data('UbiProtein',UbiProtein[k]);
				node.data('Protein_AC',Sub_AC_via_ppi[k]);
				node.data('Protein_GeneName',Sub_GeneName_via_ppi[k]);
				node.css('shape','circle'); //proteins
				node.css('background-color', 'magenta'); //Magenta color
			}
			...
			for(var j=0 ; j < sub_ID_List.length ; j++) // Add UbiSubstrate nodes
			{
				...
				node.data('UbiProtein',0);
				node.data('numPPI',sub_numPPI[j]);
			...
			// Given list contains only E3 ligases: Add E3 nodes from List of all possible UbiSubstrate
			for(var i=0 ; i < E3_ID_List2.length ; i++){
				var node = cy.nodes("[uniprotid*='"+E3_ID_List2[i]+"']");
				//node.data('text',E3_ID_List[i]);
				node.data('text',E3_GeneName_List2[i]);
				node.data('IsE3','true');
				//node.data('IsSub','false');
				node.data('UbiProtein',0);
				node.data('numPPI',0);	
				node.data('E3_GeneName',E3_GeneName_List2[i]);
				node.data('E3_Group',E3_group_List2[i]);
				node.data('E3_Ensembl',E3_Ensembl_List2[i]);
				node.data('E3_PubMedID',E3_PubMedID_List2[i]);
				node.css('shape','hexagon'); //substrate proteins
				node.css('background-color','blue');
				node.css('width','40');
				node.css('height','40');
	  ...
	  function handle_click(event) {
		  ...
		  if(event.cyTarget.isNode()==true && event.cyTarget.data('IsProtein') == "true" && event.cyTarget.data('IsSub') == "false" && event.cyTarget.data('IsE3') == "false"){
			  ...
			  print_uniprotID_AC("\tProtein_ID : ", event.cyTarget.data("uniprotid"));
			  print_uniprotID_AC("\tProtein_AC : ", event.cyTarget.data("Protein_AC"));
			  viewUbiSite_prediction(event.cyTarget.data('UbiProtein'), event.cyTarget.data("uniprotid"));
		  }
		  if(event.cyTarget.isNode()==true && event.cyTarget.data('IsSub') == "true" && event.cyTarget.data('IsE3') == "false"){
			 ...
			 // print("\tUbiSubstrate_AC : "+event.cyTarget.data("Sub_AC"));
			  print_uniprotID_AC("\tUbiSubstrate_ID : ", event.cyTarget.data("uniprotid"));
		 ...
		 if(event.cyTarget.isNode()==true && event.cyTarget.data('IsE3') == "true"){
			  ...
			  print_uniprotID_AC("\tE3_ID : ", event.cyTarget.data("uniprotid"));
	  ...
	  function viewUbiSite_prediction(isUbiProtein, ID){
		  ...
		  tmp ="<a href=http://140.138.144.145/~ubinet/viewUbiSite_Prediction.php?keyword="+ID+"  target=_blank  style=text-decoration:none;color:blue;>"+text+"</a>";
		  document.getElementById("note").innerHTML += "<p>" + msg + tmp +"</p>";
	  }
...
if(input_proteins[0] != "")
{
	...
	for(var i=0 ; i <Sub_ID_via_ppi.length ; i++)
	{
		var tmp1=cy.add({
			group: "nodes",
				data: {id:Sub_ID_via_ppi[i], name: Sub_ID_via_ppi[i], uniprotac: Sub_ID_via_ppi[i] , uniprotid: Sub_ID_via_ppi[i] , IsUbiPredicted: UbiProtein[i], IsSub:'false', IsE3:'false', shouldhide: 'true' , value:"Null", right_click: "false" , gene: Sub_GeneName_via_ppi[i]}
		});
		tmp1.data('host',tmp1.data('id'));
	}
