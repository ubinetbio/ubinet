...
for($k=0;$k<count($protein); $k++)
	if(strlen($protein[$k])>4) $d++;
if($d==count($E3_ID_List))
   for($i=0 ; $i < count($protein) ; $i++)
	  {
		  // Get list of UbiSubstrate proteins and its informations
		   $sql3 = "select distinct E3_ID, Sub_ID, Sub_UbiSites, Sub_PubID from final_Interaction_E3_Sub_PPI_Human_No_String where E3_ID = '$protein[$i]'";       
  ...
  ready: function(){
	  ...
	  function handle_click(event) {
		  ...
		  if(event.cyTarget.isNode()==true && event.cyTarget.data('IsSub') == "true"){
			  ...
			  print("Substrate_AC : "+event.cyTarget.data("Sub_AC"));
...
{
	/*
	for(var i=0 ; i <input_proteins.length ; i++){
		if(cy.nodes("[name='"+input_proteins[i]+"']").data('id') == undefined){
			...
			var tmp1=cy.add({
						group: "nodes",
						data: {parent:tmp.data('id'), name: input_proteins[i] , uniprotac: input_proteins[i] , uniprotid: input_proteins[i] , IsE3:'false', shouldhide: 'true' , value:"Null", right_click: "false" , gene: input_proteins[i]}
			});
			tmp.data('host',tmp1.data('id'));
		
		}
		else{
				var node = cy.nodes("[name='"+input_proteins[i]+"']");
				var AC = node.data('uniprotac');
				var ID = node.data('uniprotid');
				if(AC.match(input_proteins[i]) == null){
					AC = AC + " ; " + input_proteins[i];
					ID = ID + " ; " + input_proteins[i];
